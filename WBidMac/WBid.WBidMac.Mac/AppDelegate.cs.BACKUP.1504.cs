				...
				if (!string.IsNullOrEmpty (version) && version != CommonClass.AppVersion) {
					InvokeOnMainThread (() => {
						...
						alert.Buttons [0].Activated += (object senderr, EventArgs ee) => {
							...
							overlay.OverlayText = "Downloading WBidMax dmg \n Please wait..";
...
<<<<<<< HEAD
			...
			//string ss = tempPath;
			//string path=NSSearchPath.GetDirectories (NSSearchPathDirectory.DownloadsDirectory, NSSearchPathDomain.System, true).First ();
...
=======
			...
			client.DownloadFileAsync(new Uri(url),downloadPath +"/Wbid_mac.zip");
			client.DownloadFileCompleted += (object sender, System.ComponentModel.AsyncCompletedEventArgs e) => {
				InvokeOnMainThread(()=>{
					var alert = new NSAlert ();
					alert.AlertStyle = NSAlertStyle.Informational;
...
>>>>>>> c5a75085042f1bbf5b4b5dc594683faa2a636f97
				if(e.Error==null)
				{
